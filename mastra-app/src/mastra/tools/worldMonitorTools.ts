...
export const fetchEarthquakesTool = createTool({
  ...
  execute: async ({ context }) => {
    return fetchWithCache(`earthquakes-${context.limit}`, async () => {
      ...
      try {
        const res = await fetch(`${WORLD_MONITOR_BASE_URL}/worldmonitor.seismology.v1.SeismologyService/ListEarthquakes`, {
          ...
          body: JSON.stringify({ limit: context.limit })
        });
...
export const fetchRiskScoresTool = createTool({
  ...
  execute: async ({ context }) => {
    return fetchWithCache(`risk-scores-${context.region || 'global'}`, async () => {
      ...
      try {
        ...
        return { region: context.region || 'Global', instabilityIndex: 85, alertLevel: 'SEVERE', activeSanctions: 12 };
      }
    });
  }
});
...
export const fetchServerDigestTool = createTool({
  ...
  execute: async ({ context }) => {
    return fetchWithCache(`server-digest-${context.variant}`, async () => {
      console.log('📰 Fetching Server News Digest...');
      try {
        const res = await fetch(`${WORLD_MONITOR_BASE_URL}/api/news/v1/list-feed-digest?variant=${context.variant}`);
